...
public class ConfigMod {
    ...
    private static Result<Config> create(Path file) {
        ...
        return Ok()
            ...
            .then(_ -> select("API Provider Type (required):", cfg.getTypes()))
            .then(UserInput::getMessage)
            .then(cfg::setType)
            .then(_ -> readLine("API Provider Name (required):"))
            .then(UserInput::getMessage)
            .then(cfg::setProvider)
            .then(_ -> readLine("API Base URL (required):"))
            .then(UserInput::getMessage)
            .then(cfg::setApiBase)
            .then(_ -> readLine("API Key (optional):"))
            .then(UserInput::getMessage)
            .then(cfg::setApiKey)
            .then(_ -> ProviderMod.getModels(cfg.getType(), cfg.getApiBase()))
